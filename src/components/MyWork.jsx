...
const subjects = data?.filter(item => !item.isCourse);
  return (
    <div className="px-4 md:px-8 md:py-10 my-10">
      ...
      <h1 className="font-bold text-center md:text-left text-3xl">Talk is Cheap<br className="md:hidden"/><span className='hidden md-'>-</span>Here&apos;s My Work </h1>
            ...
            {courses.length > 0 ? (
              courses?.map((item) => {
                ...
                return (
                  <li
                    key={item?._id}
                    className="bg-[#171825] border-[#2a2d38] max-h-[400px] w-full max-w-[500px] p-3 rounded-xl flex-shrink-0"
                  >
                    {isVideo ? (
                      <video
